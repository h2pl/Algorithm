 ...
 * [编程|35分] String puzzle
 ...
 空间限制：2097152K
 题目描述
 Give a undirected Basic.map, each edge’s length is 1, each node has a numeric ID. There are some special nodes in the Basic.map, node’s the minimum distance to the special node is the length of shortest path to the nearest special node. Please calculate all the nodes’ minimum distance to special node. If a node could not reach any special node the distance is –1.
 输入描述:
 First line is a number N means that the next N lines are the N edges in the Basic.map. Each of the N lines has two value v1 v2 which are the two nodes’ ID linked by this edge. 0 < N < 10000000
